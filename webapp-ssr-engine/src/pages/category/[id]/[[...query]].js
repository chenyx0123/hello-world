...
const GET_CATEGORY_PAGE_GQL = gql`
  ...
  query CategoryViewGetCategoryProducts($id: ID!, $filter: WProductFilter) {
    woocommerce {
      category(id: $id) {
        id
        name
...
export const getStaticProps = async ({ params }) => {
  try {
      ...
      props: omitBy(
        {
          data: {
            woocommerce: {
              category: resp?.woocommerce?.category,
    ...
    if (Object.keys(params).length > 0)
      return {
        redirect: {
          destination: APP_ROUTES.CATEGORY({ id: params.id }),
          permanent: true,
        },
        revalidate: 60,
      };
...
export default function Page({ data, filter }) {
  ...
  const { category, products } = data?.woocommerce ?? {};
  const seoTitle =
    (search ? `${search}` : `${category.name}`) +
    (page > 1 ? ` 第${page}頁` : "");
  const canonical = `${SeoBaselineConfig.canonical}${APP_ROUTES.CATEGORY({
    id: category.id,
    search,
    page,
  })}`;
  return (
    <>
      <TrackPage
        intent={APP_PATHS.CATEGORY}
        data={{ id: category.id, page, search }}
      />
      <NextSeo
        {...SeoBaselineConfig}
        title={seoTitle}
      ...
      <ProductsView
        ...
        paginationHref={(p) =>
          APP_ROUTES.CATEGORY({ id: category.id, search, page: p })
        }
      />
    </>
  );
